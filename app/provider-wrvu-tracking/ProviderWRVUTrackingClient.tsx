...
const getInitialStateFromLocalStorage = (): ProviderWRVUTrackingState => {
  ...
  try {
    ...
    logger.error('Error loading saved state from localStorage:', error);
  }
...
export default function ProviderWRVUTrackingPage() {
  ...
  // Load from Firebase or localStorage after mount
  useEffect(() => {
    const loadState = async () => {
      // Try Firebase first if user is authenticated
      if (userId && firebaseStorage.shouldUseFirebase()) {
        try {
          const firebaseState = await firebaseStorage.loadProviderWRVUTrackingFromFirebase(userId);
          if (firebaseState) {
            // Ensure dailyData exists
            if (!firebaseState.dailyData) firebaseState.dailyData = {};
            // Ensure currentMonth exists
            if (!firebaseState.currentMonth) {
              firebaseState.currentMonth = format(startOfMonth(new Date()), 'yyyy-MM-dd');
            }
            ...
            try {
              ...
              logger.error('Error saving Firebase state to localStorage backup:', error);
            }
            
            if (firebaseState.currentMonth) {
              const date = new Date(firebaseState.currentMonth + 'T00:00:00');
          ...
          logger.error('Error loading state from Firebase, falling back to localStorage:', error);
        }
      }
      
      // Fallback to localStorage
      const savedState = getInitialStateFromLocalStorage();
      setState(savedState);
  ...
  // Save to Firebase and localStorage whenever state changes (silent auto-save, debounced)
  useEffect(() => {
    if (!userId || !firebaseStorage.shouldUseFirebase()) {
    ...
    const timeoutId = setTimeout(() => {
      firebaseStorage.saveProviderWRVUTrackingToFirebase(userId, state)
        .then(() => {
        ...
        .catch((error) => {
          logger.error('Error saving state to Firebase, falling back to localStorage:', error);
  ...
  // Also save to localStorage as immediate backup (for unauthenticated users or as backup)
  ...
  const handleManualSave = async () => {
    ...
    if (userId && firebaseStorage.shouldUseFirebase()) {
      try {
        await firebaseStorage.saveProviderWRVUTrackingToFirebase(userId, state);
      } catch (error) {
