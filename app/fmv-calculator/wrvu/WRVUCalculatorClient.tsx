...
function WRVUCalculatorPageContent() {
  ...
  const SCREEN_ID = DRAFT_SCREEN_IDS.FMV_WRVU;
      ...
      try {
        ...
        if (draft) {
          ...
          if (d.annualWrvus > 0) {
            ...
            setMonthlyBreakdown(Array.isArray(d.monthlyBreakdown) && d.monthlyBreakdown.length === 12 ? d.monthlyBreakdown : Array(12).fill(0));
            setFte(d.fte || 1.0);
            setMarketBenchmarks(d.marketBenchmarks || {});
            setActiveTab(d.activeTab === 'provider' || d.activeTab === 'market' || d.activeTab === 'results' ? d.activeTab : 'provider');
          }
        }
      } catch (error) {
