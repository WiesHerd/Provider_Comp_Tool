...
function WRVUModelerPageContentInner() {
  ...
  const SCREEN_ID = DRAFT_SCREEN_IDS.WRVU_MODELER;

  // Auto-save draft state to Firebase/localStorage whenever inputs change (debounced)
  const draftState = {
    fte,
    annualWrvus,
    monthlyWrvus,
    ...
    const loadDraft = async () => {
      try {
        ...
        if (draft) {
          ...
          if (d.annualWrvus > 0 || d.basePay > 0 || d.providerName) {
            setFte(d.fte || 1.0);
            setAnnualWrvus(d.annualWrvus || 0);
            setMonthlyWrvus(d.monthlyWrvus || 0);
            setMonthlyBreakdown(d.monthlyBreakdown || Array(12).fill(0));
            setConversionFactor(d.conversionFactor || 45.52);
            setProviderName(d.providerName || '');
            setSpecialty(d.specialty || '');
            setCustomSpecialty(d.customSpecialty || '');
            setBasePay(d.basePay || 0);
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (fte !== previousFte && previousFte > 0 && fte > 0) {
      ...
      setMonthlyBreakdown((prev: number[]) => {
        const hasMonthlyData = prev.some((val: number) => val > 0);
        if (hasMonthlyData) {
          return prev.map((val: number) => Math.round(val * scaleFactor * 100) / 100);
        }
        return prev;
      });
    }
...
function WRVUModelerPageContent() {
  return (
    <Suspense fallback={
      ...
      <WRVUModelerPageContentInner />
    </Suspense>
  );
}

export default WRVUModelerPageContent;
