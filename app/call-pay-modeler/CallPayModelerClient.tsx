...
export default function CallPayModelerPage() {
  ...
  const SCREEN_ID = DRAFT_SCREEN_IDS.CALL_PAY_MODELER;
  ...
  useEffect(() => {
    ...
    const loadDraft = async () => {
      try {
        const draft = await loadDraftState<any>(SCREEN_ID, STORAGE_KEY);
        if (draft && draft.context) {
          // Only load draft if it has meaningful data
          const context = draft.context as CallPayContext;
          if (context && ((context as any).specialty || ((context as any).providersOnCall > 0))) {
            setContext(context || DEFAULT_CONTEXT);
    ...
    void loadDraft();
  }, [scenarioLoaded, setModelingMode]);
