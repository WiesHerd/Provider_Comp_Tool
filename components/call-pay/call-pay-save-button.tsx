...
export function CallPaySaveButton({ context, tiers, impact, annualAllowableBudget, onEmailReport }: CallPaySaveButtonProps) {
...
═══════════════════════════════════════════════════════════════════════════════════`);
  ...
  const handleSave = () => {
    ...
    const scenario: ProviderScenario = {
      ...
      specialty: context.specialty,
      providerName: context.serviceLine || undefined, // Use service line as provider/group identifier
      fte: 1.0, // Call pay is typically per provider, so we use 1.0 FTE
      annualWrvus: 0, // Not applicable for call pay scenarios
      tccComponents: [callPayComponent],
      totalTcc: impact.averageCallPayPerProvider,
      normalizedTcc: impact.callPayPer1FTE,
      // Store call-pay specific data in a way that can be restored
      // We'll need to extend ProviderScenario type to include this
    ...
    scenario.callPayData = callPayData;
    ...
    <div className="flex flex-col sm:flex-row gap-2">
      <Dialog.Root open={open} onOpenChange={setOpen}>
        <Dialog.Trigger asChild>
          <Button variant="outline" className="flex-1 w-full sm:w-auto min-h-[44px] touch-target">
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-gray-900 rounded-2xl p-6 max-w-md w-[90vw] z-50 shadow-xl">
          <Dialog.Title className="text-xl font-bold mb-2">Save Call Pay Scenario</Dialog.Title>
